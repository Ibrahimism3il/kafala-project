...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const sidebar = document.getElementById('sidebar');
    const mobileMenuToggle = document.getElementById('sidebar-toggle'); // تعريف الزر
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    menuLinks.forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            // إضافة الكلاس النشط للعنصر المختار
            const menuItem = this.querySelector('.menu-item');
            if (menuItem) {
                menuItem.classList.add('active');
            }

            // لحفظ التحديد (اختياري)
            localStorage.setItem('activeMenuItem', this.getAttribute('href'));
        });
    });

    // استعادة العنصر النشط عند التحميل
    const activeHref = localStorage.getItem('activeMenuItem');
    if (activeHref) {
        const activeLink = document.querySelector(`.menu a[href="${activeHref}"]`);
        if (activeLink) {
            const activeItem = activeLink.querySelector('.menu-item');
            if (activeItem) {
