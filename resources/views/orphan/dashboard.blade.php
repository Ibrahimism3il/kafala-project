    ...
    <div class="all-bage d-flex">
        ...
        <div class="container">
            ...
            <div class="main-content">
                <div class="header">
                    <div class="welcome-message">مرحباً بك : <span>{{ auth()->user()->name }}</span> <br>
                        <p class="text-message"> نتمنى لك يوماً سعيداً ومليئاً بالنجاح.</p>
                    </div>
                </div>
                <div class="main-cards">
                    <div class="card1">
                        <p class="title-card">تفاصيل كفالتي</p>
                        <div class="underline-title"></div>

                            @if ($sponsorship)
                                <p>كفيلك الحالي : <span>{{ $sponsorship->donor->name ?? '—' }}</span></p>
                                <p>نوع الكفالة : <span>{{ $sponsorship->type }}</span></p>
                                <p>تاريخ بدء الكفالة :
                                    <span>{{ \Carbon\Carbon::parse($sponsorship->start_date)->format('d/m/Y') }}</span>
                                </p>
                                <p>الحالة :
                                    <span
                                        style="background-color: #68af73; padding: 5px 7px; border-radius: 15px; color: white;">
                                        {{ $sponsorship->status }}
                                    </span>
                                </p>
                               
                            @else
                                <h4 style="color: gray;">غير مكفول حالياً</h4>
                            @endif
                    </div>
                    ...
                    <div class="card2">
                        ...
                        <img src="{{ asset('storage/' . auth()->user()->photo) }}"
                            style="width:70px; height: 70px; border-radius: 50%; object-fit: cover; " alt="">
                        <br>
                        <div class="underline-title"></div>

                        <p> الاسم : <span>{{ $orphan->name }}</span></p>
                        <p> العمر : <span>{{ $orphan->age }} سنة</span></p>
                        <p> مكان الإقامة : <span>{{ $orphan->area }}</span></p>
                    </div>
